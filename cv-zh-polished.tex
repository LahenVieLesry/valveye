...
\begin{center}
    ...
    \contactitem{\faPhone}{+86\,139\,6232\,0052} \quad
...
\vspace{2pt}
宁波大学计算机科学与技术硕士应届生，专注\textbf{多媒体信息安全与人工智能安全}方向。硕士期间以第一作者在 \textbf{IEEE TIFS}（CCF-A / SCI 一区）投稿 2 篇、\textbf{Interspeech}（CCF-B）发表 1 篇，累计参与发表/投稿论文 \textbf{10 篇}。具备从算法设计、模型训练到工程落地的全栈研发能力。
\vspace{8pt}
\end{minipage}

\begin{tabularx}{\textwidth}{@{}p{2.8cm}X@{}}

\textbf{\color{themecolor}\faShieldAlt\;模型安全性评估研究} &
...
\textbf{\color{themecolor}\faWaveSquare\;深度学习与音频信号处理} &
\parbox[t]{\hsize}{\small
...
\textbf{\color{themecolor}\faRobot\;LLM \& Agent 工程} &
\parbox[t]{\hsize}{\small
...
\textbullet\;设计 9 个领域工具，实现意图分解、任务路由与 Agent Handoff 协作\\
...
\begin{itemize}
    ...
    \item \textbf{深度学习：}PyTorch（TorchVision / TorchAudio / DataParallel）、TensorBoard、Hugging Face
    ...
    \item \textbf{工程与平台：}\LaTeX{}、Git、GitHub、Docker、CMake、APScheduler、Rich CLI
    \item \textbf{开发工具：}VS Code、Jupyter Notebook、SSH、PyCharm、MySQL
    \item \textbf{语言能力：}CET-6（544 分）\;\textbar\; 日语（成人自考）
\end{itemize}
% \end{multicols}
...
& {\small\color{lightgray} 技术栈：LangChain + LangGraph + OpenViking + OpenAI API + Python + aiohttp + aiosqlite} \\
& \par\smallskip
\par\smallskip\hangindent=1em\textbullet\;基于 LangChain + LangGraph 构建\textbf{ Supervisor + 4 Specialist 多 Agent 架构}，设计 9 个领域工具并按 Agent 分组，实现意图分解、任务路由与 Agent 间 Handoff 协作
