...
\textbullet\;暴露 \cvtag{MCP} Server 接口（12 工具全暴露），新增 WebSearch/WebFetch 扩展信息获取\\
\textbullet\;实现交互式权限确认、操作日志审计、速率限制与用户画像反馈循环等工程化能力
} & \\
...
& {\small\color{lightgray} 技术栈：LangChain + LangGraph + OpenViking + API + MCP + Python + aiohttp + aiosqlite} \\
& \par
...
\par\smallskip\hangindent=1em\textbullet\;实现\textbf{定时价格监控}调度（APScheduler）与 多渠道富文本消息推送
\par\smallskip\hangindent=1em\textbullet\;暴露 \textbf{MCP Server} 标准接口（12 个工具全暴露），支持外部工具客户端无缝调用；新增 WebSearch/WebFetch 工具扩展信息获取能力
\par\smallskip\hangindent=1em\textbullet\;设计\textbf{结构化错误码}体系与数据源降级透明化（ITAD/SteamDB/CheapShark 链路可视化），引入异步令牌桶限流器保障 API 稳定性
\par\smallskip\hangindent=1em\textbullet\;实现\textbf{交互式权限确认}（上下键菜单）、操作日志审计（SQLite 持久化 + /audit 查询）、任务进度展示与上下文窗口可视化
\par\smallskip\hangindent=1em\textbullet\;构建\textbf{用户画像反馈循环}：记录用户对推荐结果的 like/dislike，动态调整标签权重；OpenViking 不可用时自动降级到本地文件记忆后端
\\
\end{tabularx}
